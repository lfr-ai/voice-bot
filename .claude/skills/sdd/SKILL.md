...
when_to_use: >
  Before implementing any new user-facing feature or domain behavior. Use this skill
  to write concrete scenarios in docs/specs/ that become the authoritative definition
  of done, and that drive both tests and documentation simultaneously.
paths:
  - "docs/specs/**/*.md"
  - "tests/**/*.py"
  - "backend/src/ekko/**/*.py"
...
## Scenario: Retrieve a completed transcription

**Given** a transcription with ID 42 exists and has status "completed"
**And** the transcription has 3 entries totalling 8.4 seconds
**When** the client GETs `GET /api/v1/transcriptions/42`
**Then** the response status is `200 OK`
**And** the response body contains `full_text`, `language`, and `duration_ms`
**And** `duration_ms` equals `8400.0`
```
...
Use concrete examples. Involve domain language (ubiquitous language from DDD skill).
...
1. Update the spec in `docs/specs/` first (in its own commit if helpful).
2. Update the test to match the new scenario.
3. Update the implementation.
4. All three change together.
