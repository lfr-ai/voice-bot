# AGENT.md — Voice-Bot Repository Guide
...
src/voice/
  ...
  infrastructure/   # Adapters: STT, audio streaming, OpenAI, persistence, LLM
  presentation/     # API: routes (health, stream), middleware, schemas
...
- Environment settings: `src/voice/config/settings/` (base → local/dev/test/staging/prod)
- Settings factory: `get_settings()` with `VOICE_ENVIRONMENT` env var
- Naming registry: `registry/naming_registry.json` → `src/voice/core/registry_constants.py`
