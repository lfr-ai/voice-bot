...
- `ModelMode` – indicates chat, completion, or embedding surfaces; used to select
  the adapter API.
...
- Prefer using enum values (e.g., `LLMProvider.OPENAI.value`) for external
  configuration fields.

Extending

- Add new values to `enums.py` and update any `registry/naming_registry.json`
  if using the naming-registry workflow.
